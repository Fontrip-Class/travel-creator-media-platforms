...
import { Task, TaskStatus, DashboardStats, Notification } from "@/types/database";
...
export default function SupplierDashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // 這裡會調用真實的 API
      // const dashboardData = await apiService.getSupplierDashboard();
      // const tasksData = await apiService.getSupplierTasks();
      // const statsData = await apiService.getSupplierStats();
      
      // 模擬 API 延遲
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // 更新狀態
      // setTasks(dashboardData.data.tasks || []);
      // setStats(statsData.data || MOCK_STATS);
      
      toast({
        title: "數據已更新",
        description: "儀表板數據已成功刷新",
      });
    } catch (error) {
      console.error('加載儀表板數據失敗:', error);
      toast({
        title: "加載失敗",
        description: "無法加載儀表板數據，請稍後再試",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">NT$ {stats.totalBudget.toLocaleString()}</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">5星評價</p>
                </div>
